...
export const ClassResourcesSection: React.FC<{
  ...
  if (!classResources || classResources.length === 0) return null;

  return (
    <div className="bg-slate-950 p-4 rounded-xl border border-emerald-500/30 space-y-3 shadow-xl">
      <div className="border-b border-slate-800 pb-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2">
            <span>⚡</span> Recursos de Classe ({character.class_name || 'Guerreiro'})
          </h3>
        ...
        <p className="text-xs text-slate-400 mt-1">
          Habilidades ativas, usos limitados por descanso e ações da sua classe em combate.
        </p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {classResources.map((res: any, idx: number) => {
          const resetLabel =
            res.reset === 'short'
              ? 'Descanso Curto'
              : res.reset === 'long'
          ...
          const actionCost =
            res.action ||
            (res.name.includes('Fúria') || res.name.includes('Fôlego')
              ? 'Ação Bônus'
              : 'Ação Principal');
          ...
          return (
            <div
              ...
              className="bg-slate-900 border border-slate-800/90 hover:border-emerald-500/40 p-3 rounded-xl flex flex-col justify-between transition shadow-md gap-2"
            >
